...
SELECT title FROM sakila.film WHERE language_id NOT IN 
(SELECT language_id FROM sakila.language WHERE name = "English" OR name = "Japanese")

##Part two
#You were just hired at a company as a data analyst, and your company needs some information from the database. 
#They would like to see all payments that had a rental amount is over .99 cents. However, they only want to see the 
#rental id and payment date that is attached to that payment. Once you have finished that, they would like to see the 
#staff id and customer id from the payments that have an amount over .99 cents.
